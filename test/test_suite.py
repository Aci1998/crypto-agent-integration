...
class AgentIntegrationTester:
    ...
    def test_price_service(self) -> bool:
        """测试价格服务是否运行"""
        try:
            ...
            if response.status_code == 200:
                ...
                if data.get('status') == 'healthy':
                    self.log_test("价格服务连接", True, "服务正常运行")
                    return True
                else:
                    self.log_test("价格服务连接", False, "服务状态异常")
                    return False
            else:
                self.log_test("价格服务连接", False, f"HTTP {response.status_code}")
                return False
        except requests.exceptions.ConnectionError:
            self.log_test("价格服务连接", False, "无法连接到价格服务，请先运行 python price_service.py")
            return False
        except Exception as e:
            self.log_test("价格服务连接", False, str(e))
            return False
    ...
    def test_crypto_agent(self) -> bool:
        ...
        try:
            ...
            # 测试价格查询
            result = agent.process_query("BTC价格")
    ...
    def test_api_endpoints(self) -> bool:
        ...
        try:
            ...
            response = requests.post(f"{self.api_base_url}/api/crypto/batch", 
                                   json=batch_data, timeout=10)
            if response.status_code == 200:
                data = response.json()
                if "BTC" in data and "ETH" in data:
                    self.log_test("批量API查询", True, "批量查询成功")
                else:
                    self.log_test("批量API查询", False, "批量查询响应异常")
                    success = False
            else:
                self.log_test("批量API查询", False, f"HTTP {response.status_code}")
                success = False
    ...
    def test_mcp_server(self) -> bool:
        ...
        try:
            ...
            if not os.path.exists("mcp_server.py"):
                self.log_test("MCP服务器文件", False, "mcp_server.py 不存在")
                return False
            
            # 测试配置生成
            result = subprocess.run([
                sys.executable, "mcp_server.py", "config"
    ...
    def test_integrations(self) -> bool:
        ...
        # 测试LangChain集成
        try:
            from integrations.langchain_integration import create_langchain_tool
            tool = create_langchain_tool()
            if tool:
                self.log_test("LangChain集成", True, "LangChain工具创建成功")
            else:
                self.log_test("LangChain集成", False, "LangChain工具创建失败")
                success = False
        except Exception as e:
            self.log_test("LangChain集成", False, str(e))
            success = False
        
        # 测试OpenAI集成
        try:
            from integrations.openai_integration import get_openai_function_schema, handle_openai_function_call
            
            schema = get_openai_function_schema()
            if "name" in schema and "parameters" in schema:
                self.log_test("OpenAI Schema", True, "函数schema生成成功")
                ...
                self.log_test("OpenAI Schema", False, "Schema格式异常")
                success = False
                
        except Exception as e:
            self.log_test("OpenAI集成", False, str(e))
            success = False
...
def main():
    ...
    try:
        ...
        if success:
            print("\n🚀 快速开始:")
            print("1. 启动价格服务: python price_service.py")
            print("2. 测试Agent: python crypto_agent.py")
            print("3. 配置MCP: python mcp_server.py config")
            print("4. 查看集成示例: examples/")
        else:
            print("\n🔧 故障排除:")
            print("1. 确保价格服务正在运行")
            print("2. 检查网络连接")
